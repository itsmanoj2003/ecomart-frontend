...
export default function AdminOffers() {
  ...
  const [quantity] = useState(['1 Piece', '1 Packet', '1 Bottle', '50 g', '100 g', '250 g', '500 g', '750 g', '1 Kg', '1/2 Kg', '1/4 Kg', '3/4 Kg', '2 Kg', '3 Kg', '4 Kg', '5 Kg', '10 Kg', '15 Kg', '20 Kg', '25 Kg', '50 ml', '100 ml', '250 ml', '500 ml', '750 ml', '1 Lit', '1/2 Lit', '1/4 Lit', '3/4 Lit','2 Lit','3 Lit','4 Lit','5 Lit']);
  ...
  const fetchOffers = async () => {
    try {
      const res = await axios.get('http://localhost:3001/ecomart/getoffers');
      setOffers(res.data || []);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await axios.put(`http://localhost:3001/ecomart/updateoffer/${editingId}`, formData);
        alert("Offer Updated Successfully");
        setEditingId(null);
      } else {
        // create new offer
        await axios.post('http://localhost:3001/ecomart/postoffers', formData);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:3001/ecomart/deleteoffer/${id}`);
